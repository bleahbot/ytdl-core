...
const { dumpLastPlayer } = require("./sig");
/**
 * @param {string} link
...
const downloadFromInfoCallback = (stream, info, options) => {
  ...
  let __dumped = false;
  const __dumpIfZero = async (reason) => {
  if (__dumped) return;
    if (downloaded === 0) {
      try { await dumpLastPlayer(reason); } catch (_) {}
    }
    __dumped = true;
  };

  stream.once("end",    () => { __dumpIfZero("zerobytes"); });
  stream.once("error",  () => { __dumpIfZero("stream_error"); });
  stream.once("close",  () => { __dumpIfZero("closed"); });

  if (process.env.YTDL_TEST_ZERO === "1") {
