...
exports.checkForUpdates = () => {
  if (
    ...
    return exports
      .request("https://api.github.com/repos/bleahbot/ytdl-core/contents/package.json", {
        requestOptions: {
          headers: {
            "User-Agent":
      ...
      .then(
        response => {
          ...
          if (pkgFile.version !== pkg.version && updateWarnTimes++ < 5) {
            ...
            console.warn(
              '\\x1b[33m[warn]\x1B[0m @bleah/ytdl-core is out of date! Update with "npm install @bleah/ytdl-core@latest".',
            );
          }
        },
        err => {
...
exports.saveDebugFile = (name, body) => {
  if (process.env.YTDL_NO_DEBUG_FILE) {
    console.warn(`\\x1b[33m[warn]\x1B[0m Debug file saving is disabled. "${name}"`);
    return body;
  }
  ...
  if (!options.agent) {
    ...
    if (c) {
      ...
      if (oldCookieWarning) {
        ...
        console.warn(
          "\\x1b[33m[warn]\x1B[0m Using old cookie format, " +
            "please use the new one instead. (https://github.com/bleahbot/ytdl-core#cookies-support)",
        );
      }
    }
    if (options.requestOptions.dispatcher && oldDispatcherWarning) {
      ...
      console.warn(
        "\\x1b[33m[warn]\x1B[0m Your dispatcher is overridden by `ytdl.Agent`. " +
          "To implement your own, check out the documentation. " +
          "(https://github.com/bleahbot/ytdl-core#how-to-implement-ytdlagent-with-your-own-dispatcher)",
      );
    }
...
exports.applyOldLocalAddress = options => {
  ...
  if (oldLocalAddressWarning) {
    ...
    console.warn(
      "\\x1b[33m[warn]\x1B[0m Using old localAddress option, " +
        "please add it to the agent options instead. (https://github.com/bleahbot/ytdl-core#ip-rotation)",
    );
  }
};
...
exports.applyIPv6Rotations = options => {
  if (options.IPv6Block) {
    ...
    if (oldIpRotationsWarning) {
      ...
      oldLocalAddressWarning = false;
      console.warn(
        "\\x1b[33m[warn]\x1B[0m IPv6Block option is deprecated, " +
          "please create your own ip rotation instead. (https://github.com/bleahbot/ytdl-core#ip-rotation)",
      );
    }
  }
};
